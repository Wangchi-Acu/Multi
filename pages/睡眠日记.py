...
# 工具：20:00-02:00 每分钟刻度
TIME_SLOTS = [f"{h:02d}:{m:02d}" for h in range(20, 24) for m in range(60)] + \
...
with st.form("sleep_diary"):
    ...
    nap_start = col1.select_slider("昨日白天小睡开始时间", options=TIME_SLOTS, value="14:00")
    nap_end   = col2.select_slider("昨日白天小睡结束时间", options=TIME_SLOTS, value="14:20")
    ...
    med_time = med_col3.select_slider("服用时间", options=TIME_SLOTS, value="22:00")
    ...
    bed_time       = st.select_slider("昨晚上床时间", options=TIME_SLOTS, value="23:00")
    try_sleep_time = st.select_slider("试图入睡时间", options=TIME_SLOTS, value="23:15")
...
if submitted and name.strip():
    ...
    conn.commit()
    conn.close()
    st.success(f"{date.today()} 睡眠日记已保存！")
